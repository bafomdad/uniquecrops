...
public class TileMusicaPlant extends TileBaseUC implements ITickable {
	...
	public void update() {
		...
		for (int i = 0; i < beats.size(); i++) {
			...
			int elapsedtime = (int)beat.getTimeElapsed(world.getTotalWorldTime(), beat.getTime());
			...
			if (!timelist.isEmpty() && !beats.isEmpty()) {
				if (world.getTotalWorldTime() % 10 == 0 && lastBeat > 0) {
					int randomtick = (world.rand.nextInt(100) + 5) - timelist.size();
					if (this.getPercussion() != null && (randomtick < this.getPercussion().getTimeElapsed(world.getTotalWorldTime(), lastBeat)))
					{
						((Musica)this.blockType).addAge(world, getPos(), state, 1);
						return;
					}
				}
			}
	...
	public void listenAndRemove(Beat beat, int index) {
		...
		long diff = beat.getTimeElapsed(world.getTotalWorldTime(), beat.getTime());
		if (diff > 50) {
