...
public class WorldGenSkyTree extends WorldGenAbstractTree {
	
    private static final IBlockState LOG = UCBlocks.flywoodLog.getDefaultState();
    private static final IBlockState LEAF = UCBlocks.flywoodLeaves.getDefaultState().withProperty(BlockOldLeaf.CHECK_DECAY, Boolean.valueOf(false));
    private final boolean useExtraRandomHeight;

    public WorldGenSkyTree(boolean notify, boolean useExtraRandomHeightIn) {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            if (!flag)
            {
                ...
                if (position.getY() < worldIn.getHeight() - i - 1) {
                    ...
                    this.setBlockAndNotifyAdequately(worldIn, position, LOG);
    ...
    protected boolean canGrowInto(Block block) {
    	...
    	if (block != UCBlocks.flywoodSapling)
