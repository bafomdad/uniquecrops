...
public class Malleatoris extends BlockCropsBase {
	...
	private boolean calcGrowth(EntityItem stack, Random rand) {
		...
		if (!stack.getEntityItem().getItem().isRepairable()) return false;
		
		double damage = stack.getEntityItem().getMaxDamage() * 0.1;
		double weight = damage / 2;
		double chance = Math.random() * 100;
		if (chance < weight) {
			int newDamage = (int)damage + stack.getEntityItem().getItemDamage();
			if (newDamage < stack.getEntityItem().getMaxDamage()) {
				stack.getEntityItem().setItemDamage(stack.getEntityItem().getItemDamage() + (int)damage);
				UCPacketHandler.sendToNearbyPlayers(stack.worldObj, stack.getPosition(), new PacketUCEffect(EnumParticleTypes.CLOUD, stack.posX, stack.posY, stack.posZ, 6));
				return true;
			}
		}
		return false;
